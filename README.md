...
HEV 的运行时 YAML 同时声明 `multi-queue: true`、固定 IPv4/IPv6 地址，并使用
`icmp: reply` 接管 ICMP Echo。Android 传入现成的单个 `VpnService` TUN 文件描述符，
因此接口地址、路由和实际队列数量仍以 `VpnService.Builder` 创建的接口为准。
