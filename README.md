...
覆写面板可选择 `silent`、`error`、`warning`、`info` 或 `debug`，默认使用 `warning`。选择结果只在
内存中的 mihomo 配置生效，不改写上传的 `config.yaml`。

WebUI 默认仅监听 `127.0.0.1`。开启“向局域网公开 WebUI”后改为监听 `0.0.0.0`，面板会显示实际
端口；其他设备使用 `http://<手机局域网 IP>:<端口>/ui/` 访问。控制器优先使用端口 `17890`，冲突
时选择同一监听接口上的可用临时端口。Android 17（API 37）会在应用该开关时请求
`ACCESS_LOCAL_NETWORK`；未授权时不保存新的公开设置，也不会重启 VPN。权限之后被撤销时，下次
核心重建会把 WebUI 自动恢复为 `127.0.0.1`，不会因可选的 LAN 功能中断 VPN。

控制器鉴权直接使用用户 `config.yaml` 中的 `secret`，应用不再生成或覆盖密码，也不提供重复的密码
输入。向局域网公开时 `secret` 必须非空，否则拒绝启动；仅本机模式允许沿用 mihomo 对空 `secret`
的原始行为。
...
- 使用覆写面板选择的日志级别，默认 `warning`；
- WebUI 默认监听 `127.0.0.1`，按开关切换为 `0.0.0.0`，并保留 YAML 中的 `secret`；
