...
当前 Wi-Fi/移动网络提供的 DNS。普通上游 socket 执行 protect 后跟随 Android 默认物理出口，
保护失败会终止该次拨号。仅 System WebView XHTTP 绑定指定 Network。

上游 DNS 答案不能包含本机 fake-IP 地址池中的地址；这类新答案会被拒绝，旧缓存会在读取时
淘汰并重新查询。普通 DNS 清理同时覆盖 bootstrap 缓存，不清除应用仍在使用的 fake-IP 映射。
...
用户连接意愿会同步保存。开机解锁及应用更新后，在授权仍有效时自动恢复此前未主动停止的 VPN。
RMX5010 实测系统可能以 `o-kill(40) ... with swipe up` 杀死前台 VPN，然后仅向新服务交付
`onTaskRemoved` 而不调用 `onStartCommand`；这种空实例会按保存的连接意愿恢复核心。已运行的实例不重启。
这属于被杀后的恢复，不能保住已被系统销毁的 TUN 或现有连接。通知同时设置 ongoing 和 NO_CLEAR。
