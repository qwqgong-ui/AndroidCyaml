...
```
默认进程  代理:VpnService、两个原生库、Go runtime
:ui       界面:面板与 WebView
:log      日志:诊断文件与采样时钟
```

采样器此前住在代理进程里,紧挨着它要测量的运行时。这让它和 VPN 共享生命周期——而这恰恰是
记录者最不该做的事:核心重启或低内存击杀正是证据最有价值的时刻,而随被记录对象一起死掉的
记录者什么都留不下。

同时,日志文件的潜在写者有几个进程就有几个。两个 2 MiB 世代在两个写者下轮转是必然丢数据
的:追加会在行中间交错,更糟的是两个进程可能同时决定轮转,丢掉对方正在追加的那一代。

现在:

- `:log` 进程持有文件和时钟,自身不持有任何运行时;
- 代理与 UI 进程不再写文件,而是把行交给 `DiagnosticsRelay` 的有界内存缓冲;
- `:log` 每分钟通过一次 binder 调用 `captureDiagnostics`,一次取走指标、内核日志行与中继
  事件,然后落盘。一分钟一次往返,而不是每事件一次——后者会把 IPC 放在风暴期间每分钟运行
  数千次的路径上;
- `DiagnosticsLog.append` 的调用点一处未改:行落在哪里由进程角色在一处决定。

## 顺带修掉的

**采样器不再创建任何线程**,时钟归 `:log` 了。这直接消除了此前观察到的线程堆积——真机上曾
见到 4 个同名采样线程,其中多数完全惰性。

**stop 与 start 的竞态。** 旧的 `stop()` 把 `DiagnosticsLog.setEnabled(false)` 推迟到工作
线程执行。紧接着的 `start()` 会先把日志置为开启,随后旧线程排队的 runnable 才把它改回关闭
——采样器自以为在运行,实际什么都没写。唯一的症状是空日志,而空日志看起来和"功能没开"
一模一样。现在关闭动作同步执行。

**线程名截断。** `/proc` 的 `comm` 截断到 15 字符,`AndroidCyaml-diagnostics` 与
`AndroidCyaml-diagnostics-export` 在该长度下完全相同,线程普查无法区分它们——而这正是普查
会被问到的问题。改名为 `acy-diag-sample`、`acy-diag-export`、`acy-log-worker`,均在限长内
且互不相同。

## 线程普查

诊断采样加入 `thread.<名字>=<数量>`,复用 `artAttachFloor` 已有的 `/proc/self/task` 遍历,
不增加 I/O。ART 的 `Thread-N` 归为一条,只输出计数 ≥2 的名字。它就是定位上述线程堆积的
工具——第一次采样就直接给出了 `thread.acy-diag-sample=4`。

# AndroidCyaml v1.0.54 发布说明

## 线程普查

`threads=N` 只说数量涨了,不说涨的是谁。真机上出现过 7 个同名线程,其中 5 个完全惰性
(1–19 次上下文切换、零 CPU),而代码里只有两处创建该名字的线程、且其中一处在 `:ui` 进程。
从外部无法确认,因为 `/proc` 的 `comm` 截断到 15 字符,而
`AndroidCyaml-diagnostics` 与 `AndroidCyaml-diagnostics-export` 的前 15 字符完全相同。

本版让这个问题自己回答:

- 两个诊断线程改名为 `acy-diag-sample` 与 `acy-diag-export`,都在 15 字符以内且互不相同,
  内核报告的名字从此可区分;
- 诊断采样加入线程普查,按内核报告的名字计数,输出 `thread.<名字>=<数量>`。它复用
  `artAttachFloor` 已有的那趟 `/proc/self/task` 遍历,不增加任何 I/O;
- ART 自己的 `Thread-N` 归为一条 `thread.Thread-N`,它们已由 attach 高水位概括,逐个列出
  会是数千条描述同一件事的记录;
- 只输出计数 ≥2 的名字。一个进程有几十个一次性线程,逐个列出会淹没真正在增长的池。

这是一次让工具能回答问题的改动,不是对某个已定位缺陷的修复——那 7 个线程的来源仍未确认,
下一次采样会直接说出它们叫什么。

## 说明

上一版把 mihomo 的日志收进诊断日志的改动已实测生效:诊断日志中现在同时存在
`event=sample` 的指标行与逐条 `event=core` 的内核日志行,处于同一时间线。
